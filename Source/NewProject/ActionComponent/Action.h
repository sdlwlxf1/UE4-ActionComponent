...
class FAction : public TSharedFromThis<FAction>
{
	...
	DECLARE_DELEGATE_TwoParams(FPostFinish, FAction*, EActionResult);
...
protected:
	...
	virtual bool FinishAction(EActionResult InResult, EActionType StopType = EActionType::Default) { return true; }
	...
	void NotifyActionFinish(EActionResult Result);
	void NotifyTypeChanged();
	virtual void UpdateType() {}

	virtual bool FinishChildAction(FAction* InAction, EActionResult InResult, EActionType StopType = EActionType::Default) { return true; }

	EActionType Type;

private:
	bool DoFinishAction(EActionResult InResult, EActionType StopType = EActionType::Default);
